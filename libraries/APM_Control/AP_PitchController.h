...
private:
    ...
    float _last_pid_info_D = 0.0f;          // value of the D term from the previous time step (deg)
    LowPassFilterFloat _slew_rate_filter;   // LPF applied to the derivative of the control action generated by the pitch rate feedback
    float _slew_rate_amplitude = 0.0f;      // Amplitude of the servo slew rate produced by the pitch rate feedback (deg/sec)
    float _D_gain_modifier = 1.0f;          // Gain modifier applied to the pitch rate feedback to prevent excessive slew rate
    AP_Float _slew_rate_max;                // Maximum permitted slew rate (deg/sec)
